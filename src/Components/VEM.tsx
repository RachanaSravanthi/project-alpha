...
const VimeoEmbed: React.FC<VimeoEmbedProps> = ({ link, className }) => {
  ...
  const handleMouseEnter = () => {
    ...
    if (iframeRef.current) {
      iframeRef.current.src = `${link}?controls=0&title=0&byline=0&portrait=0&autoplay=1`;
    }
  };
  ...
  const handleMouseLeave = () => {
    ...
    if (iframeRef.current) {
      iframeRef.current.src = `${link}?controls=0&title=0&byline=0&portrait=0`;
    }
  };
  ...
  return (
    <iframe
      ref={iframeRef}
      src={`${link}?controls=0&title=0&byline=0&portrait=0`}
