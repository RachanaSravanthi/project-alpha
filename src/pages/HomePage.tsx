    ...
    return (
        <div className="min-h-screen bg-black text-white overflow-x-hidden font-inter">
                ...
                <nav className="w-full">
                    <ul className="flex justify-center items-center space-x-8">
                        ...
                        <motion.li variants={fadeIn}>
                            <button onClick={() => scrollTo('about')} className="hover:text-gray-600 transition-colors duration-300 cursor-pointer">About</button>
                        </motion.li>
                        <motion.li variants={fadeIn}>
                            <button onClick={() => scrollTo('work')} className="hover:text-gray-600 transition-colors duration-300 cursor-pointer">Work</button>
                        </motion.li>
                        <motion.li variants={fadeIn}>
                            <button onClick={() => scrollTo('contact')} className="hover:text-gray-600 transition-colors duration-300 cursor-pointer">Let's talk</button>
                        </motion.li>
                    </ul>
                </nav>
            </motion.header>
            ...
            <main className="container mx-auto px-4">
                ...
                >
                    <motion.h1 variants={fadeIn} className="text-2xl md:text-4xl font-bold mb-4">Hello! I'm Rachana Sravanti</motion.h1>
                    <motion.h2 variants={fadeIn} className="text-3xl md:text-5xl font-bold mb-8">
                        Bringing Imagination to Life:<br />
                        Stunning VFX, 2D & 3D Designs<br />
                        for Film and Beyond!
                    </motion.h2>
                    <motion.div variants={fadeIn} className="flex flex-col md:flex-row justify-center space-y-4 md:space-y-0 md:space-x-4">
                        <button onClick={() => scrollTo('contact')} className="bg-white text-black px-6 py-2 rounded-full hover:bg-gray-200 transition-colors duration-300 w-full md:w-auto">Let's talk</button>
                        <button onClick={() => scrollTo('work')} className="border border-white px-6 py-2 rounded-full hover:bg-white hover:text-black transition-colors duration-300 w-full md:w-auto">My works</button>
                    </motion.div>
                </motion.section>
